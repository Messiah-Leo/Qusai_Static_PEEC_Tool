...
int Save_Netlist() {
	...
	if (!fin) {
		Console::Error("Cannot open " + Console::ShortPath(inputFile));
		return 1;
	}
	...
	if (!fout) {
		Console::Error("Cannot create " + Console::ShortPath(outputFile));
		return 1;
	}
	...
	fout.close();
	Console::Info("Stable HSPICE netlist generated: " + Console::ShortPath(outputFile));
	return 0;
}
