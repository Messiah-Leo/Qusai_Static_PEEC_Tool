...
#include <complex>
#include <iostream>
#include <mkl_service.h>     // 直接包含以避免通过传递包含引发的警告
#include <stdexcept>
#include <string>
#include <type_traits>
#include <vector>

// 不在头文件中进行顶层函数调用。提供一个内联辅助函数以便在需要时设置线程数。
// 调用示例： SetMKLThreads(4);
inline void SetMKLThreads(int n = 50) {
	...
	int mat_type)
{
	...
	if constexpr (std::is_same_v<T, double>) {
		...
		static_assert(std::is_same_v<T, double> || std::is_same_v<T, std::complex<double>>, "Solve_M only supports double and std::complex<double>.");
	}
	...
	int method_key)
{
	...
	if constexpr (std::is_same_v<T, double>) {
		...
		static_assert(std::is_same_v<T, double> || std::is_same_v<T, std::complex<double>>, "Inverse_M only supports double and std::complex<double>.");
	}
