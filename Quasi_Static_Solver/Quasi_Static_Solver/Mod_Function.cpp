...
void Time_Diff(double t1, double t2) {
	Console::Info("Completed in " + Console::FormatDurationMs(std::fabs(t2 - t1)));
	Console::Blank();
}
...
{
	const std::string ll_bin = File_Path + "LL_OO.bin";
	const std::string pp_bin = File_Path + "PP_OO.bin";
	const std::string ll_txt = File_Path + "LL_OO.txt";
	const std::string pp_txt = File_Path + "PP_OO.txt";

	if (std::ifstream(ll_bin, std::ios::binary).good() && std::ifstream(pp_bin, std::ios::binary).good()) {
		if (!Read_Matrix_From_Binary_File(ll_bin, LL)) return false;
		if (!Read_Matrix_From_Binary_File(pp_bin, PP)) return false;
	}
	else {
		if (!Read_Matrix_From_File(ll_txt, LL)) return false;
		if (!Read_Matrix_From_File(pp_txt, PP)) return false;
	}

	if (!Read_Matrix_From_File(File_Path + "B2N.txt", CN)) return false;
	std::ifstream fin(File_Path + "PORT.txt");
	if (!fin.is_open()) {
		Console::Error("Cannot open file: " + Console::ShortPath(File_Path + "PORT.txt"));
		return false;
	}

	std::string line;
	if (!std::getline(fin, line)) {
		Console::Error("PORT.txt is empty or malformed.");
		return false;
	}

	std::stringstream ss(line);
	if (!(ss >> N_PORT)) {
		Console::Error("Failed to parse port count.");
		return false;
	}
	...
	for (int i = 0; i < N_PORT; ++i) {
		if (!std::getline(fin, line)) {
			Console::Error("PORT.txt has fewer rows than expected.");
			return false;
		}

		std::stringstream line_ss(line);
		if (!(line_ss >> PORT_DATA[i].N[0] >> PORT_DATA[i].N[1] >> PORT_DATA[i].Zin)) {
			Console::Error("Invalid port row at line " + std::to_string(i + 2) + ".");
			return false;
		}
	}

	fin.close();
	Console::Detail("Ports loaded", N_PORT);
	return true;
}
